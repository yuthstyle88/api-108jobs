-- Add monthly and half yearly active columns for site and category aggregates
...
FROM
    category_aggregates_activity ('1 day') mv
WHERE
    ca.category_id = mv.category_id_;
...
FROM
    category_aggregates_activity ('1 week') mv
WHERE
    ca.category_id = mv.category_id_;
...
FROM
    category_aggregates_activity ('1 month') mv
WHERE
    ca.category_id = mv.category_id_;
...
FROM
    category_aggregates_activity ('6 months') mv
WHERE
    ca.category_id = mv.category_id_;
