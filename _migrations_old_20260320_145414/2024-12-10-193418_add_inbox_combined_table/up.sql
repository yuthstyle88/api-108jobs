...
INSERT INTO inbox_combined (published, comment_reply_id, person_comment_mention_id, person_post_mention_id)
SELECT published,
       id,
       NULL::int, NULL::int
FROM comment_reply
...
SELECT published,
       NULL::int, id,
       NULL::int
FROM person_comment_mention
UNION ALL
SELECT published,
       NULL::int, NULL::int, id
FROM person_post_mention;
...
ALTER
CONSTRAINT person_post_mention_recipient_id_fkey NOT DEFERRABLE,
    ALTER
CONSTRAINT person_post_mention_post_id_fkey NOT DEFERRABLE,
    ADD CONSTRAINT person_post_mention_unique UNIQUE (recipient_id, post_id);

-- Make sure only one of the columns is not null
ALTER TABLE inbox_combined
    ADD CONSTRAINT inbox_combined_check CHECK (num_nonnulls(comment_reply_id, person_comment_mention_id,
                                                            person_post_mention_id) = 1) ,
ALTER
CONSTRAINT inbox_combined_comment_reply_id_fkey NOT DEFERRABLE,
    ALTER
CONSTRAINT inbox_combined_person_comment_mention_id_fkey NOT DEFERRABLE,
    ALTER
CONSTRAINT inbox_combined_person_post_mention_id_fkey NOT DEFERRABLE;