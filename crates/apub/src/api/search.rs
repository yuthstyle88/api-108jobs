...
use app_108jobs_api_utils::utils::{check_conflicting_like_filters, check_fetch_limit, check_private_instance};
...
pub async fn search(
  ...
  // Load logistics for post results
  let (viewer, is_admin) = match &local_user_view {
    Some(luv) => {
      ...
      let viewer = if is_admin {
        ...
        LogisticsViewer::Public // We don't have post creator info here, so use Public
      };
      (viewer, is_admin)
    }
  ...
  load_logistics_for_results(&mut results, pool, viewer, is_admin).await?;
