...
use lemmy_utils::{
  error::{FederationError, FastJobContext, FastJobContext},
...
impl Object for ApubComment {
  ...
  async fn delete(self, context: &Data<Self::DataType>) -> FastJobContext<()> {
    if !self.deleted {
  ...
  async fn into_json(self, context: &Data<Self::DataType>) -> FastJobContext<Note> {
  ...
  async fn from_json(note: Note, context: &Data<FastJobContext>) -> FastJobContext<ApubComment> {
...
pub(crate) mod tests {
  ...
  async fn prepare_comment_test(
    url: &Url,
    context: &Data<FastJobContext>,
  ) -> FastJobContext<(ApubPerson, ApubCommunity, ApubPost, ApubSite)> {
  ...
  #[tokio::test]
  #[serial]
  pub(crate) async fn test_parse_lemmy_comment() -> FastJobContext<()> {
  ...
  #[tokio::test]
  #[serial]
  async fn test_parse_pleroma_comment() -> FastJobContext<()> {
    let context = FastJobContext::init_test_context().await;
    let test_data = TestData::create(&mut context.pool()).await?;
    let url = Url::parse("https://enterprise.lemmy.ml/comment/38741")?;
    let data = prepare_comment_test(&url, &context).await?;
