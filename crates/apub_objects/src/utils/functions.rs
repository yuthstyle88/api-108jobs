...
use lemmy_utils::{
  error::{FederationError, LemmyError, FastJobContext},
  CacheLock,
...
pub async fn local_site_data_cached(pool: &mut DbPool<'_>) -> FastJobContext<Arc<LocalSiteData>> {
...
pub async fn check_apub_id_valid_with_strictness(
  ...
  if is_strict && !local_site_data.allowed_instances.is_empty() {
    ...
    if !allowed_and_local.contains(&domain) {
      Err(FastJobErrorType::FederationDisabledByStrictAllowList)?
    }
  }
  Ok(())
}
...
pub fn check_apub_id_valid(apub_id: &Url, local_site_data: &LocalSiteData) -> FastJobContext<()> {
  let domain = apub_id
    .domain()
...
pub fn generate_to(community: &Community) -> FastJobContext<Vec<Url>> {
...
pub fn verify_is_public(to: &[Url], cc: &[Url]) -> FastJobContext<()> {
...
pub fn verify_visibility(to: &[Url], cc: &[Url], community: &ApubCommunity) -> FastJobContext<()> {
