...
impl FederationBlockList {
  pub async fn block(pool: &mut DbPool<'_>, form: &FederationBlockListForm) -> FastJobResult<Self> {
    let conn = &mut get_conn(pool).await?;
    insert_into(federation_blocklist::table)
      .values(form)
      .get_result::<Self>(conn)
      .await
      .with_fastjob_type(FastJobErrorType::CouldntBlockInstance)
  }
  pub async fn unblock(pool: &mut DbPool<'_>, instance_id_: InstanceId) -> FastJobResult<usize> {
    let conn = &mut get_conn(pool).await?;
