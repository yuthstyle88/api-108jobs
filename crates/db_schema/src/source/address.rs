...
fn validate_address(form: &AddressForm) -> Result<(), FastJobError> {
  if form.address_line1.trim().is_empty() {
    return Err(FastJobErrorType::ValidationError("Address line 1 cannot be empty".to_string()).into());
  }

  if form.subdistrict.is_none(){
    return Err(FastJobErrorType::ValidationError("Subdistrict cannot be empty".to_string()).into());
  }
  ...
  if form.postal_code.trim().is_empty() {
    return Err(FastJobErrorType::ValidationError("Postal code cannot be empty".to_string()).into());
  }
