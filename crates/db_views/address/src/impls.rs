...
impl AddressView {
  pub async fn find_by_id(pool: &mut DbPool<'_>, address_id: AddressId) -> FastJobResult<Self> {
...
impl TryFrom<UpsertAddressRequest> for UpsertAddress {
  ...
  fn try_from(form: UpsertAddressRequest) -> Result<Self, Self::Error> {
    let address_line1 =
      get_required_trimmed(&form.address_line1, FastJobErrorType::EmptyAddressLine1)?;
