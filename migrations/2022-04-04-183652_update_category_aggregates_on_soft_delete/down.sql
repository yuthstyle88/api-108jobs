DROP TRIGGER IF EXISTS category_aggregates_post_count ON post;

DROP TRIGGER IF EXISTS category_aggregates_comment_count ON comment;
...
DROP TRIGGER IF EXISTS site_aggregates_category_insert ON category;

DROP TRIGGER IF EXISTS site_aggregates_category_delete ON category;
...
CREATE OR REPLACE FUNCTION category_aggregates_post_count ()
    RETURNS TRIGGER
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION site_aggregates_category_insert ()
    RETURNS TRIGGER
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION site_aggregates_category_delete ()
    RETURNS TRIGGER
    LANGUAGE plpgsql
    AS $$
BEGIN
